...
# Only include these series
allowed_series = {"base", "neo", "ex", "swsh"}
...
for serie_item in os.listdir(data_folder):
    ...
    # Filter by allowed series
    if serie_id not in allowed_series:
        continue
    ...
    for set_item in os.listdir(serie_path):
        ...
        for file in os.listdir(set_path):
            ...
            if types_match:
                types_list = [t.strip().strip('"') for t in types_match.group(1).split(",") if t.strip()]
            else:
                types_list = []
    ...
    for condition, cond_data in price_data.get("data", {}).items():
        ...
        for date, values in history.items():
            price_rows.append({
                ...
                "name": card["name"],
                "rarity": card["rarity"],
                "types": ",".join(card["types"]),  # <-- add types as comma-separated string
