...
function createViolinPlot(data, selector) {
  ...
  grouped.forEach((values, rarity) => {
    ...
    // --- Add vertical guide lines for each X label ---
    violinG.selectAll(".x-guide").remove(); // clean old ones

    violinG.selectAll(".x-guide")
      .data(rarities)
      .enter()
      .append("line")
      .attr("class", "x-guide")
      .attr("x1", d => violinX(d) + violinX.bandwidth() / 2)
      .attr("x2", d => violinX(d) + violinX.bandwidth() / 2)
      .attr("y1", 0)                     // optional: start at top of plot
      .attr("y2", violinInnerHeight)     // down to X-axis
      .attr("stroke", "#000000ff")
      .attr("stroke-width", 1)
      .attr("stroke-dasharray", "3"); // optional dashed line
